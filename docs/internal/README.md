# Internal Documentation

This section contains technical documentation for developers and contributors of Lazymvn.

## Implementation Details

*   [Implementation Summary](./IMPLEMENTATION_SUMMARY.md)
*   [Live Reload Implementation](./LIVE_RELOAD_IMPLEMENTATION.md)
*   [Log Formatting Implementation](./LOG_FORMATTING_IMPLEMENTATION.md)

## Refactoring

*   [Refactoring](./REFACTORING.md)
*   [Refactoring Priorities](./REFACTORING_PRIORITIES.md)
*   [Refactoring Summary](./REFACTORING_SUMMARY.md)
*   [Phase 6 - Micro-Refactoring](./PHASE6_MICRO_REFACTORING.md)
*   [Phase 7.1 - Plan](./PHASE_7.1_PLAN.md)

## Bug Fixes

*   [Fix: Profile Loading for New Tab](./FIX_PROFILE_LOADING_NEW_TAB.md)
*   [Fix: Shared Starter Cache Across Tabs](./FIX_SHARED_STARTER.md)

## Tabs Feature

*   [Tabs Proposal](./TABS_PROPOSAL.md)
*   [Tabs Implementation](./TABS_IMPLEMENTATION.md)
*   [Tabs Progress](./TABS_PROGRESS.md)
*   [Tabs Phase 2 Migration](./TABS_PHASE2_MIGRATION.md)

## Testing

*   [Test Coverage Analysis](./TEST_COVERAGE_ANALYSIS.md)
*   [Test Coverage Progress](./TEST_COVERAGE_PROGRESS.md)

## Old README

*   [Old README](./OLD_README.md)
