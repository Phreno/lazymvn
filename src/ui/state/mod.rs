...
impl TuiState {
    ...
    /// Yank (copy) debug logs from current session to clipboard
    pub fn yank_logs(&mut self) {
        log::info!("Attempting to yank debug logs");

        // Get logs from current session
        let logs_result = crate::logger::get_current_session_logs();
        
        let logs_text = match logs_result {
            Ok(logs) => {
                if logs.is_empty() {
                    log::info!("No logs available to copy");
                    let tab = self.get_active_tab_mut();
                    tab.command_output.push(String::new());
                    tab.command_output.push("⚠ No logs available (debug mode not enabled?)".to_string());
                    return;
                }
                logs
            }
            Err(e) => {
                ...
                let tab = self.get_active_tab_mut();
                tab.command_output.push(String::new());
                tab.command_output.push(format!("✗ Failed to retrieve logs: {}", e));
                return;
            }
        };

        let line_count = logs_text.lines().count();
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if let Ok(mut child) = Command::new("wl-copy").stdin(Stdio::piped()).spawn()
                && let Some(mut stdin) = child.stdin.take()
                && stdin.write_all(logs_text.as_bytes()).is_ok()
            {
                drop(stdin);
                if child.wait().is_ok() {
                    log::info!("Copied {} lines of logs via wl-copy", line_count);
                    let tab = self.get_active_tab_mut();
                    tab.command_output.push(String::new());
                    tab.command_output
                        .push(format!("✓ Copied {} lines of debug logs to clipboard", line_count));
                    return;
                }
            }
                ...
                && stdin.write_all(logs_text.as_bytes()).is_ok()
            {
                drop(stdin);
                if child.wait().is_ok() {
                    log::info!("Copied {} lines of logs via xclip", line_count);
                    let tab = self.get_active_tab_mut();
                    tab.command_output.push(String::new());
                    tab.command_output
                        .push(format!("✓ Copied {} lines of debug logs to clipboard", line_count));
                ...
                && stdin.write_all(logs_text.as_bytes()).is_ok()
            {
                drop(stdin);
                if child.wait().is_ok() {
                    log::info!("Copied {} lines of logs via xsel", line_count);
                    let tab = self.get_active_tab_mut();
                    tab.command_output.push(String::new());
                    tab.command_output
                        .push(format!("✓ Copied {} lines of debug logs to clipboard", line_count));
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            {
                if let Some(mut stdin) = child.stdin.take() {
                    if stdin.write_all(logs_text.as_bytes()).is_ok() {
                        drop(stdin);
                        if child.wait().is_ok() {
                            log::info!("Copied {} lines of logs via PowerShell", line_count);
                            let tab = self.get_active_tab_mut();
                            tab.command_output.push(String::new());
                            tab.command_output
                                .push(format!("✓ Copied {} lines of debug logs to clipboard", line_count));
            ...
            if let Ok(mut child) = Command::new("pbcopy").stdin(Stdio::piped()).spawn() {
                if let Some(mut stdin) = child.stdin.take() {
                    if stdin.write_all(logs_text.as_bytes()).is_ok() {
                        drop(stdin);
                        if child.wait().is_ok() {
                            log::info!("Copied {} lines of logs via pbcopy", line_count);
                            let tab = self.get_active_tab_mut();
                            tab.command_output.push(String::new());
                            tab.command_output
                                .push(format!("✓ Copied {} lines of debug logs to clipboard", line_count));
        ...
        // Fallback to arboard if all system tools failed
        let clipboard_result = if let Some(ref mut clipboard) = self.clipboard {
            clipboard.set_text(logs_text)
        } else {
            match arboard::Clipboard::new() {
                Ok(mut clipboard) => {
                    let result = clipboard.set_text(logs_text);
                    self.clipboard = Some(clipboard);
                    result
                }
                Err(e) => {
        ...
        match clipboard_result {
            Ok(()) => {
                log::info!("Copied {} lines of logs to clipboard via arboard", line_count);
                tab.command_output.push(String::new());
                tab.command_output
                    .push(format!("✓ Copied {} lines of debug logs to clipboard", line_count));
            }
            Err(e) => {
                log::error!("Failed to copy logs to clipboard: {}", e);
                tab.command_output.push(String::new());
                tab.command_output.push(format!("✗ Failed to copy logs: {}", e));
            }
        }
    }
