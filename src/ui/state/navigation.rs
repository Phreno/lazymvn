...
pub(super) fn is_navigation_debounced(
...
pub(super) fn calculate_next_index(current: Option<usize>, list_len: usize) -> usize {
    match current {
        Some(i) => (i + 1) % list_len,
        None => 0,
    }
...
pub(super) fn calculate_previous_index(current: Option<usize>, list_len: usize) -> usize {
    match current {
        Some(i) => {
            if i == 0 {
                list_len - 1
