...
impl Theme {
    ...
    pub const SELECTED_STYLE: Style = Style::new().bg(Color::LightBlue).add_modifier(Modifier::BOLD);
    
    /// Style for active profiles (with asterisk)
    pub const ACTIVE_PROFILE_STYLE: Style = Style::new().fg(Color::Green).add_modifier(Modifier::BOLD);
